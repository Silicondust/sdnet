 ...
 * dns_lookup_new.c
 *
 * Copyright © 2012-2013,2020 Silicondust USA Inc. <www.silicondust.com>.  All rights reserved.
 *
...
struct dns_manager_t dns_manager;
...
uint8_t dns_entry_get_dns_ip_index(struct dns_entry_t *dns_entry)
{
	if ((dns_entry->state != DNS_ENTRY_STATE_PRIMARY) && (dns_entry->state != DNS_ENTRY_STATE_SECONDARY)) {
		DEBUG_ERROR("state error");
		return 0xFF;
	}

	return (dns_entry->state == DNS_ENTRY_STATE_PRIMARY) ? 0 : 1;
}
...
struct netbuf *dns_manager_generate_request_nb(struct dns_entry_t *dns_entry)
{
...
static void dns_manager_recv_record_a(struct dns_entry_t *dns_entry, ipv4_addr_t ip_addr, uint32_t time_to_live)
{
	...
	if (dns_ip_index != 0xFF) {
		dns_manager_update_dns_entry_connection_state(dns_entry, dns_ip_index, DNS_ENTRY_CONNECTION_STATE_COMPLETE);
	}

	dns_entry->ip_addr = ip_addr;
	dns_entry->state = DNS_ENTRY_STATE_COMPLETE;
...
void dns_manager_recv_common(struct netbuf *nb)
{
...
static void dns_manager_timer_callback(void *arg)
{
	...
	struct dns_entry_t *dns_entry = slist_get_head(struct dns_entry_t, &dns_manager.dns_list);
	while (dns_entry) {
		if ((current_time >= dns_entry->expire_time) && !slist_get_head(struct dns_lookup_t, &dns_entry->dns_lookup_list)) {
			struct dns_entry_t *dns_discard = dns_entry;
			dns_entry = slist_get_next(struct dns_entry_t, dns_entry);
			heap_free(slist_detach_item(struct dns_entry_t, &dns_manager.dns_list, dns_discard));
			continue;
		}

		if (dns_entry->state == DNS_ENTRY_STATE_PRIMARY) {
			if (current_time >= dns_entry->expire_time) {
				dns_entry->connection_state[0] = DNS_ENTRY_CONNECTION_STATE_ERROR;
			}
			
			if (dns_entry->connection_state[0] == DNS_ENTRY_CONNECTION_STATE_ERROR) {
				dns_entry->state = DNS_ENTRY_STATE_SECONDARY;
				dns_entry->expire_time = current_time + (ticks_t)DNS_ENTRY_EXPIRE_TIME_PENDING_SEC * TICK_RATE;
				dns_entry_send_request(dns_entry);
			}
		}

		if (dns_entry->state == DNS_ENTRY_STATE_SECONDARY) {
			if (current_time >= dns_entry->expire_time) {
				dns_entry->connection_state[1] = DNS_ENTRY_CONNECTION_STATE_ERROR;
			}

			if (dns_entry->connection_state[1] == DNS_ENTRY_CONNECTION_STATE_ERROR) {
				dns_entry->state = DNS_ENTRY_STATE_COMPLETE;
				dns_entry->expire_time = current_time + (ticks_t)DNS_ENTRY_EXPIRE_TIME_FAILED_SEC * TICK_RATE;
			}
		}

		if ((dns_entry->state == DNS_ENTRY_STATE_COMPLETE) && !dns_lookup_to_signal) {
...
bool dns_lookup_gethostbyname(struct dns_lookup_t *dns_lookup, const char *name, dns_lookup_gethostbyname_callback_t callback, void *callback_arg)
{
	...
	if (dns_entry) {
		...
		if (dns_entry->state != DNS_ENTRY_STATE_COMPLETE) {
...
void dns_manager_update_dns_entry_connection_state(struct dns_entry_t *dns_entry, uint8_t dns_ip_index, uint8_t connection_state)
{
	dns_entry->connection_state[dns_ip_index] = connection_state;
	dns_manager_update_timer();
}
...
void dns_manager_set_server_ip(ipv4_addr_t dns_ip_primary, ipv4_addr_t dns_ip_secondary)
{
	...
	dns_manager.dns_ip[0] = dns_ip_primary;
	dns_manager.dns_ip[1] = dns_ip_secondary;
}

void dns_manager_init_common(void)
{
