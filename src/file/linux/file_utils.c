...
{
	int fp = open(path, flags | O_LARGEFILE | O_NOCTTY | O_NOFOLLOW, FILE_CREATE_MODE);
	if (fp < 0) {
		return NULL;
	}
...
{
	return (lseek64(file->fp, (off64_t)offset, SEEK_SET) >= 0);
}

bool file_seek_advance(struct file_t *file, uint64_t offset)
{
	return (lseek64(file->fp, (off64_t)offset, SEEK_CUR) >= 0);
}
...
{
	return (lseek64(file->fp, 0, SEEK_END) >= 0);
}

uint64_t file_get_pos(struct file_t *file, uint64_t result_on_error)
{
	off64_t result = lseek64(file->fp, 0, SEEK_CUR);
	if (result < 0) {
...
bool file_move(const char *new_path, const char *old_path)
{
	...
	int new_fp = open(new_path, O_RDWR | O_CREAT | O_TRUNC | O_LARGEFILE | O_NOCTTY | O_NOFOLLOW, FILE_CREATE_MODE);
	if (new_fp < 0) {
		close(old_fp);
		free(buffer);
		return false;
